    ...
    <div className="flex items-center gap-2 text-sm">
      {isActive ? (
        <p className="text-[#8C8C8C] text-[14px]">
          Didn't get OTP? <span className='text-[#3B3C9A]'>Resend in{" "}</span>
          <span className="text-[#3B3C9A] font-medium">
            {seconds}s
          </span>
        </p>
      ) : (
        <button
          ...
          className="text-[#5759D0] hover:underline"
        >
          Resend OTP
        </button>
      )}
...
export default function ForgotPasswordTwo({}) {
    ...
    return (
        <>
        <section id='ForgotPassword(Page2)' className="relative flex flex-col items-center justify-center translate-y-24">
            <button className='text-white flex gap-2 items-center absolute w-[5opx] h-12  inset-0 -translate-y-1/2 left-15'
                onClick={() => navigate(-1)}
            >
                <LeftArrow />
            ...
            <div className='p-3 lg:w-[600px] mx-auto flex-1 lg:h-[784px]'>
                    <div className='flex lg:p-12 lg:px-8 rounded-[8px] mt-[8rem] flex-col gap-6 bg-gradient-to-r from-[#100E24] to-[#100E24] border border-[#3B3C9A99] p-4'>
                        ...
                        <div className='flex flex-col gap-2 lg:mt-5 text-[24px]'>
                            <h1 classname=''>Verify Code </h1>
                            <p className='text-white opacity-50 lg:opacity-50 text-[14px] w-full'>We've sent a 6-digit code to your email.</p>
                        </div>
                        
                        
                            <div className='flex flex-col gap-4'>
                                <div className='gap-4 w-full text-white lg:flex lg:gap-4 lg:max-w-full'>
                                    <OtpInput
                                        value={otp}
                                        ...
                                        renderInput={(props) => <input {...props} 
                                             ...
                                             style={{
                                                 ...
                                                 onFocus={(e) => {
                                                    e.target.style.border = "1px solid #3B3C9A";
                                                    e.target.style.boxShadow = "0 0 2px #1B1EFF";
                                                    e.target.style.outline = "none";
                                                }}
                                                onBlur={(e) => {
                                                    e.target.style.border = "1px solid #303030";
                                                    e.target.style.boxShadow = "0 0 2px #1B1EFF";
                                                    e.target.style.outline = "none";
                                                }}
                                        />
                            ...
                            <div className='flex flex-col gap-4 items-center'>
                                        <Link 
                                        className={`w-full text-center text-white text-[14px] rounded-[12px] px-4 py-[10px] transition duration-200
                                            ${otp.trim() ? 'bg-[#3B3C9A]' :'bg-[#7C7CBB]'}
                                        ` 
                                        }
                                        type='button'
                                        to='/reset-password'
